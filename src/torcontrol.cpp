...
TorControlConnection::TorControlConnection(struct event_base *baseIn):
    base(baseIn), b_conn(0)
{
}
...
bool TorControlConnection::Connect(const std::string &target, const ConnectionCB& connectedIn, const ConnectionCB& disconnectedIn)
{
    if (b_conn)
        Disconnect();
...
TorController::TorController(struct event_base* baseIn, const std::string& targetIn):
    base(baseIn),
    target(targetIn), conn(base), reconnect(true), reconnect_ev(0),
...
void TorController::add_onion_cb(TorControlConnection& connIn, const TorControlReply& reply)
{
    if (reply.code == 250) {
...
void TorController::auth_cb(TorControlConnection& connIn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        connIn.Command(strprintf("ADD_ONION %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
...
void TorController::authchallenge_cb(TorControlConnection& connIn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            connIn.Command("AUTHENTICATE " + HexStr(computedClientHash), boost::bind(&TorController::auth_cb, this, _1, _2));
        } else {
...
void TorController::protocolinfo_cb(TorControlConnection& connIn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            ...
            connIn.Command("AUTHENTICATE", boost::bind(&TorController::auth_cb, this, _1, _2));
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                // connIn.Command("AUTHENTICATE " + HexStr(status_cookie.second), boost::bind(&TorController::auth_cb, this, _1, _2));
                cookie = std::vector<uint8_t>(status_cookie.second.begin(), status_cookie.second.end());
                clientNonce = std::vector<uint8_t>(TOR_NONCE_SIZE, 0);
                GetRandBytes(&clientNonce[0], TOR_NONCE_SIZE);
                connIn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&TorController::authchallenge_cb, this, _1, _2));
            } else {
...
void TorController::connected_cb(TorControlConnection& connIn)
{
    reconnect_timeout = RECONNECT_TIMEOUT_START;
    // First send a PROTOCOLINFO command to figure out what authentication is expected
    if (!connIn.Command("PROTOCOLINFO 1", boost::bind(&TorController::protocolinfo_cb, this, _1, _2)))
        LogPrintf("tor: Error sending initial protocolinfo command\n");
}

void TorController::disconnected_cb(TorControlConnection& connIn)
{
